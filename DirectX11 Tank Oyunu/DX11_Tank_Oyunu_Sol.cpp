...
//                    DirectX 11 Tank Oyunu Uygulamasý Çözümü
//
//--------------------------------------------------------------------------------------
...
bool canMove(vector<intersection> intersections);
...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
    ...
    g_hWnd = CreateWindow( L"TutorialWindowClass", L"DirectX 11 Tank Oyunu Uygulamasý Çözümü", WS_OVERLAPPEDWINDOW,
...
void DetectInput(double time)
{
	...
	if (mouseCurrState.rgbButtons[0] & 0x80)			// Left Button is PRESSED
	{
		...
		if (canShoot(intersections)) renderEnemy = false;
